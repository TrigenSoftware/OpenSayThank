...
function ENV(e){return("undefined"!=typeof DEV?"/app_dev.php":"")+e}function AUTH(e){e.error&&"access denied"==e.error&&(location.href=ENV("/login"))}function killObservers(e){observersStorage.forEach(function(r){e&&r.group!=e||clearInterval(r.id)})}function runObserver(e,r){r(),observersStorage.push({group:e,id:setInterval(r,2e3)})}var ThankApp=angular.module("thankApp",["ngRoute"]),apiUrl=ENV("/api"),VIEWS={INDEX:1,PROFILE:5,FOLLOWS:2,FOLLOWERS:3,SEARCH:4},INDEX_VIEWS={FEED:1,SAID:2,GOT:3},MODAL_STATE={HIDDEN:0,THANK:1,MESSAGE:2},observersStorage=[];Number.prototype.toDualDigit=function(){var e=this+"";return 1===e.length?"0"+e:e},ThankApp.config(["$routeProvider",function(e){e.when("/",{templateUrl:"/templates/index.html",controller:"IndexViewController"}).when("/follows",{templateUrl:"/templates/follows.html",controller:"FollowsViewController"}).when("/followers",{templateUrl:"/templates/followers.html",controller:"FollowersViewController"}).when("/user/:username/follows",{templateUrl:"/templates/follows.html",controller:"FollowsViewController"}).when("/user/:username/followers",{templateUrl:"/templates/followers.html",controller:"FollowersViewController"}).when("/search",{templateUrl:"/templates/search.html",controller:"SearchViewController"}).when("/user/:username",{templateUrl:"/templates/profile.html",controller:"ProfileViewController"}).when("/search/:query",{templateUrl:"/templates/search.html",controller:"SearchViewController"}).otherwise({redirectTo:"/"})}]),ThankApp.directive("gtImageUpload",function(){return{restrict:"A",scope:{image:"=gtImageUpload"},link:function(e,r){r.bind("change",function(r){var t=new FileReader,n=r.target.files[0];t.onload=function(r){e.$apply(function(){e.image=r.target.result})},0==n.type.indexOf("image")&&t.readAsDataURL(n)})}}}),ThankApp.directive("gtLoadNext",function(){return{restrict:"A",scope:{gtLoadNext:"&"},link:function(e){var r=document.body;angular.element(window).scroll(function(){r.scrollHeight-r.offsetHeight-r.scrollTop-window.innerHeight<100&&e.$apply(function(){e.gtLoadNext()})})}}}),ThankApp.controller("MainController",["$scope","$http","$location","$sce",function(e,r,t,n){e.modalState=MODAL_STATE.HIDDEN,e.modal={title:"",body:"",thankto:"",attachment:""},e.currentUserData={},e.searchQuery="",e.trustAsHtml=n.trustAsHtml,e.humanTiming=function(e){e=new Date(e);var r=new Date-e;if(6e4>r)return"меньше минуты назад";if(36e5>r){var t=Math.floor(r/6e4),n=t+"";return n=1*n[n.length-1],1===t?"минуту назад":1==n&&t>20?t+" минуту назад":2!=n&&3!=n&&4!=n||!(t>20||10>t)?t+" минут назад":t+" минуты назад"}if(864e5>r){var o=Math.floor(r/36e5),n=o+"";return n=1*n[n.length-1],1===o?"час назад":1==n&&o>20?o+" час назад":2!=n&&3!=n&&4!=n||!(o>20||10>o)?o+" часов назад":o+" часа назад"}return e.toLocaleDateString("ru-RU",{year:"numeric",month:"long",day:"numeric"})+" "+e.getHours().toDualDigit()+":"+e.getMinutes().toDualDigit()},e.search=function(){t.path("/search/"+e.searchQuery)},e.logout=function(){localStorage.clear(),location.href=ENV("/logout")},e.showSayThankModal=function(r){e.modalState=MODAL_STATE.THANK,e.modal={title:"Сказать Спасибо",body:"Текст сообщения",thankto:r&&r.username||"",userData:r,attachment:""}},e.hideModal=function(){e.modalState=MODAL_STATE.HIDDEN,e.modal={title:"",body:"",thankto:"",attachment:""},e.removeAttachment()},e.removeAttachment=function(){e.modal.attachment="",angular.element("[gt-image-upload]").val("")},e.sayThank=function(){e.modal.userData&&e.modal.userData.gotThanks++,r.post(apiUrl,{action:"sayThank",thankto:e.modal.thankto,body:e.modal.body,attachment:e.modal.attachment}).success(function(r){return AUTH(r),r.error?e.modal.userData.gotThanks--:void e.hideModal()})},e.removeThank=function(e){e.removed=!0,r.post(apiUrl,{action:"removeThank",id:e.id}).success(function(r){return AUTH(r),r.error?e.removed=!1:void 0})},e.giveFive=function(e){e.fived=!0,e.gotFives++,r.post(apiUrl,{action:"giveFive",id:e.id}).success(function(r){AUTH(r),r.error&&(e.fived=!1,e.gotFives--)})},e.removeFive=function(e){e.fived=!1,e.gotFives--,r.post(apiUrl,{action:"removeFive",id:e.id}).success(function(r){AUTH(r),r.error&&(e.fived=!0,e.gotFives++)})},e.follow=function(e){e&&e.username&&(e.follow=!0,e.followers++,r.post(apiUrl,{action:"follow",username:e.username}).success(function(r){AUTH(r),r.error&&(e.follow=!1,e.followers--)}))},e.unfollow=function(e){e&&e.username&&(e.follow=!1,e.followers--,r.post(apiUrl,{action:"unfollow",username:e.username}).success(function(r){AUTH(r),r.error&&(e.follow=!0,e.followers++)}))},r.post(apiUrl,{action:"currentUserData"}).success(function(r){AUTH(r),r.error||(e.currentUserData=r,localStorage.currentUsername=r.username)})}]),ThankApp.controller("IndexViewController",["$scope","$http","$routeParams",function(e,r){killObservers(),e.$parent.currentView=VIEWS.INDEX,e.$parent.searchQuery="",e.currentView=null,e.thanks=[],e.fromOffset=null,runObserver("currentUser",function(){r.post(apiUrl,{action:"currentUserData"}).success(function(r){AUTH(r),r.error||(e.currentUserData=r,localStorage.currentUsername=r.username)})}),e.setIndexView=function(t){if(t!=e.currentView){killObservers("thank"),e.thanks=[],e.fromOffset=null,e.currentView=t;var n;switch(t){case INDEX_VIEWS.FEED:n="thanksWithData";break;case INDEX_VIEWS.SAID:n="saidThanksWithData";break;case INDEX_VIEWS.GOT:n="gotThanksWithData"}runObserver("thank",function(){r.post(apiUrl,{action:n,username:e.$parent.currentUserData.username||localStorage.currentUsername,from:e.thanks[0]?e.thanks[0].id:e.fromOffset}).success(function(r){AUTH(r),r.error||(e.thanks=r.concat(e.thanks))})}),e.$parent.loadNext=function(){e.thanks[e.thanks.length-1]&&e.fromOffset!=e.thanks[e.thanks.length-1].id&&(e.fromOffset=e.thanks[e.thanks.length-1].id,r.post(apiUrl,{action:n,username:e.$parent.currentUserData.username||localStorage.currentUsername,from:-e.fromOffset}).success(function(r){AUTH(r),r.error||(e.thanks=e.thanks.concat(r))}))}}},e.setIndexView(INDEX_VIEWS.FEED)}]),ThankApp.controller("ProfileViewController",["$scope","$http","$location","$routeParams",function(e,r,t,n){return killObservers(),n.username==e.$parent.currentUserData.username?t.path("/"):(e.$parent.currentView=VIEWS.PROFILE,e.$parent.searchQuery="",e.userData={},e.currentView=null,e.thanks=[],e.fromOffset=null,e.setIndexView=function(t){if(t!=e.currentView){killObservers("thank"),e.thanks=[],e.fromOffset=null,e.currentView=t;var n;switch(t){case INDEX_VIEWS.FEED:n="thanksWithData";break;case INDEX_VIEWS.SAID:n="saidThanksWithData";break;case INDEX_VIEWS.GOT:n="gotThanksWithData"}runObserver("thank",function(){r.post(apiUrl,{action:n,username:e.userData.username,from:e.thanks[0]?e.thanks[0].id:e.fromOffset}).success(function(r){AUTH(r),r.error||(e.thanks=r.concat(e.thanks))})}),e.$parent.loadNext=function(){e.thanks[e.thanks.length-1]&&e.fromOffset!=e.thanks[e.thanks.length-1].id&&(e.fromOffset=e.thanks[e.thanks.length-1].id,r.post(apiUrl,{action:n,username:e.userData.username,from:-e.fromOffset}).success(function(r){AUTH(r),r.error||(e.thanks=e.thanks.concat(r))}))}}},void runObserver("user",function(){r.post(apiUrl,{action:"userData",username:n.username}).success(function(r){AUTH(r),r.error||(e.userData=r,e.thanks.length||e.setIndexView(INDEX_VIEWS.FEED))})}))}]),ThankApp.controller("FollowsViewController",["$scope","$http","$location","$routeParams",function(e,r,t,n){killObservers(),n.username==e.$parent.currentUserData.username&&t.path("/follows"),n.username||(e.$parent.currentView=VIEWS.FOLLOWS),e.$parent.searchQuery="",e.follows=[],e.fromOffset=null,r.post(apiUrl,{action:"follows",username:n.username||e.$parent.currentUserData.username||localStorage.currentUsername,from:e.fromOffset}).success(function(r){AUTH(r),r.error||(e.follows=r)})}]),ThankApp.controller("FollowersViewController",["$scope","$http","$location","$routeParams",function(e,r,t,n){killObservers(),n.username==e.$parent.currentUserData.username&&t.path("/followers"),n.username||(e.$parent.currentView=VIEWS.FOLLOWERS),e.$parent.searchQuery="",e.followers=[],e.fromOffset=null,r.post(apiUrl,{action:"followers",username:n.username||e.$parent.currentUserData.username||localStorage.currentUsername,from:e.fromOffset}).success(function(r){AUTH(r),r.error||(e.followers=r)})}]),ThankApp.controller("SearchViewController",["$scope","$http","$routeParams",function(e,r,t){killObservers(),e.$parent.currentView=VIEWS.SEARCH,e.searchResult=[],e.fromOffset=null,e.$parent.searchQuery=t.query||"",r.post(apiUrl,{action:"searchUserWithData",query:e.$parent.searchQuery,from:e.fromOffset}).success(function(r){AUTH(r),r.error||(e.searchResult=r)})}]);